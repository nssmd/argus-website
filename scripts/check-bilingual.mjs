...
for (const page of ["projects/runtime/index.html", "zh/projects/runtime/index.html"]) {
  ...
  assert((html.match(/class="project-detail-card project-detail-card--runtime"/g) || []).length === 4,
    `${page} must include Argus, Argus-Pi, CrystalPilot and Argus Verticals`);
  const verticals = html.match(/<article\b[^>]*id="argus-verticals"[^>]*>[\s\S]*?<\/article>/)?.[0] ?? "";
  assert(verticals.includes("Argus-AiTeam/argus-verticals/releases/tag/v0.1.0") &&
    verticals.includes("17") && verticals.includes("MIT"),
    `${page} lacks the independently released community domain packs`);
  ...
  assert(crystal.includes("v0.1.6") && (crystal.includes("host integration") || crystal.includes("宿主集成")),
...
for (const page of ["get-started/index.html", "zh/get-started/index.html", "projects/runtime/index.html", "zh/projects/runtime/index.html"]) {
  ...
  assert(windowsNotes.includes("Ask this step") || windowsNotes.includes("问这一步"),
    `${page} must describe ordinary reading questions in the Windows release`);
  assert(windowsNotes.includes("Problem background notes remain an opt-in experimental preview") || windowsNotes.includes("问题基础说明仍是需显式启用的实验预览"),
  ...
  for (const [english, chinese] of [
    ...
    ["read-only views cannot submit", "只读视图不能提交新问题"],
    ...
    ["Saving a proposal does not launch experiments or change the backlog", "保存方案不会启动实验或改变 backlog"],
    ["not proof of model-weight training", "不是模型权重训练的证明"],
    ["default unpriced-cost policy can stop later own-account calls", "默认的未定价费用策略可能阻止后续自有账户调用"],
  ]) {
...
for (const page of ["get-started/index.html", "zh/get-started/index.html"]) {
  ...
  for (const [id, filename, size, checksum] of [
    ...
    assert(card.includes(`releases/download/v${version}/${filename}`) &&
      card.includes(`/v${version}/SHA256SUMS`) && card.includes(`Argus ${version} ·`),
      `${page} has an incorrect ${id} URL, checksum link or accessible label`);
    assert(card.includes(size) && card.includes(checksum), `${page} has stale ${id} integrity metadata`);
  }
